...
/*#ifdef NEOPIXEL_BKGD_LED_INDEX
  ...
  void NeoPixel::set_color_background() {
...
void NeoPixel::set_color_startup(const uint32_t color) {
  for (uint16_t i = 0; i < pixels(); ++i)
    set_pixel_color(i, color);
  show();
}
...
void NeoPixel::init() {
  ...
  adaneo1.begin();
  adaneo1.show();  // initialize to all off

  #ifdef NEOPIXEL_STARTUP_TEST
...
#if 0
bool NeoPixel::set_led_color(const uint8_t r, const uint8_t g, const uint8_t b, const uint8_t w, const uint8_t p) {
